...
func FindAndCountActivity(ranbowUserId uint, _cond ActivityFindCondition) (*ActivityQueryResult, error) {
	...
	// 未开始 starttime>now
	// 进行中 (starttime<now || starttime==-1) && (endtime>now || endtime==-1) && (results.count<amount || results.count is null || amount==-1)
	// 已结束 (endtime<now && endedtime!=-1) || (results.count>=amount && results.count is not null && amount!=-1)
		...
		for _, item := range _cond.ActivityStatus {
			...
			case enums.ACTIVITY_STATUS_ONGOING:
				orClause = orClause.Or(db.
					Where(db.Where("results.minted_count < activities.amount").Or("results.minted_count is null").Or("activities.amount = -1")).
					Where("started_time <? or started_time=-1", now).
					Where("ended_time >? or ended_time=-1", now))
			case enums.ACTIVITY_SINGLE_END:
				orClause = orClause.Or(db.
					Or(db.Where("results.minted_count>=activities.amount").Where("results.minted_count is not null").Where("activities.amount!=-1")).
